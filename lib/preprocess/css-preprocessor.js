...
module.exports = inherit({
    ...
    _processUrls: function(data, filename) {
        var _this = this;
        return data
            .replace(/(?:@import\s*)?url\((?:"(.*?)"|'(.*?)'|(.*?))\)/g, function(s, urlInDoubleQuotes, urlInSingleQuotes, url) {
                if (~s.indexOf('@import') || /^(https?:|data:|\/)/.test(urlInDoubleQuotes || urlInSingleQuotes || url))
                    return s;

                return 'url(' + (
                        (urlInDoubleQuotes && '"' + _this._resolveCssUrl(urlInDoubleQuotes, filename) + '"') ||
                        (urlInSingleQuotes && "'" + _this._resolveCssUrl(urlInSingleQuotes, filename) + "'") ||
                        (url && _this._resolveCssUrl(url, filename)) || ''
                    ) + ')';
